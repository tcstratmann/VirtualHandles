...
using namespace sio;
 ...
 double string_to_double( const std::string& s )
 {
   ...
   double x;
   if (!(i >> x))
     throw 20; // TODO: sinnvolle exception werfen
   return x;
 }
...
std::string const logSpeed = "log-speed";
std::string const gpsLat = "gps-coord-north";
std::string const gpsLong = "gps-coord-east";
...
void VirtualHandles::send()
{
    ...
    sendParameter(logSpeed, (round(2.0 * model->getSpeed())));
    
    sendParameter(gpsLat, model->getLat());
    sendParameter(gpsLong, model->getLong());
